...
class Sidebar extends Component {
    ...
    routesFileBtn(elem, event){  
        ...
        this.state.clickTime = setTimeout(() => {
            if( this.state.treeFilter.filter ===  SHOW_SCAN_TREE ){
                elem['get_filter'] = 2
            } else {
                elem['get_filter'] = 0
            }                    
    ...
    scanSelectBtn(elem, isDrive, event){
        try{
            ...
            if( hasClass(p_elem, 'checked') ){
                if( hasClass(p_elem, 'hasScanPath') ){
                    ...
                    data.map((elem, index) => {
                        if( !elem.is_file ){
                            ...
                            return <li key={index} className="menu-item" style={wobj}>
                                      {
                                          this.state.beginguide == 0 && lastDrPath === elem.path ?
                                      ...
                                      <p className={p_classes} data-left={_Left} 
                                         ...
                                         <span className="item-text">{elem.volume_name ? elem.volume_name : elem.file_name}</span>
                                      </p>
    ...
    render() {
      ...
      const { isShowFilter, treeFilter } = this.state  
        ...
        if( nextProps.uncheck && nextProps.uncheckLastUpdated !== this.props.uncheckLastUpdated ){
            ...
            $p_Elem.each((index, elem) => {
                ...
                if( path === nextProps.uncheck.path ){
                    ...
                    $(elem).siblings('div.submenu').find('p.item-p').removeClass('checked hasScanPath')                  
                ...
                case 'device_change_msg':
                    if( jsData && jsData.error_code == 0  && jsData.data ){
                        ...
                        if( route ){
                            ...
                            route.menu === LOCAL_FILE_TOTAL_PANELS || hasPath ? this.inItTotalPanel(this.state.treeFilter, 'MOBILE-HARD-DISK') : null
                        }
                        this.inItAsyncgetFileRequest(this.state.treeFilter, 'MOBILE-HARD-DISK')
    ...
    shouldComponentUpdate(nextProps, nextState) {
        if( nextProps.jsMsgHandle && nextProps.jsMsgHandleLastUpdated !== this.props.jsMsgHandleLastUpdated ){
            if( nextProps.jsMsgHandle.module !== 'device_change_msg' &&
                nextProps.jsMsgHandle.module !== 'scan_init_status_t' ){
        ...
        if( nextProps.files && this.props.files && 
            ...
            if( nextProps.files && nextProps.files.common && nextProps.files.common.mode !== 0 ){ 
                log('Sidebar----------------shouldComponentUpdate：阻止渲染了1')
                return false
            }
        }
        if( nextProps.checkLastUpdated !== this.props.checkLastUpdated || 
            ...
            log('Sidebar----------------shouldComponentUpdate：阻止渲染了2')    
            return false
        }
